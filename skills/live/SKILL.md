...
5. **Pre-flight checklist** (say it once, short):
   ...
   - Pick a source: *Pestaña* for a browser call (Meet, Teams web, Zoom web) or *Pantalla + sistema* for desktop Zoom/Teams. **Chrome's picker has an audio checkbox and it must be ticked** — sharing a surface without it gives video and no transcript, which is the single most common failure. Then *Micro* so their own answers become context.
   - *Auto* answers every interviewer question; *Responder ahora* (⌥↵) forces one; *Resolver lo que se ve* (⌥S) attaches the current screen and asks for a full solution; *Deep* switches the brain to Opus for hard problems; *Flotar* pops the answer card into an always-on-top window.
   - Paste the job description under *Contexto* if `/interview-ai:prep` was not run.
...
- Transcript empty while the interviewer talks → look at the channel strips. `sin captura` means no source; `sin tramas` means the audio graph never started; `muda` means a surface was shared without its audio checkbox — re-pick and tick it. A **window** share never carries audio; a tab or a whole screen does.
